...
class ReportCSVReturKeluaranController(http.Controller):  
    ...
    def retur_keluar_report_csv(self, **kwargs):
        ...
        if data_ids: # check if data_ids not empty
            ...
            for data_id in id_list:
                ...
                datark["KD_JENIS_TRANSAKSI"] = '"' + str(move.l10n_id_tax_number[0:2] or 0) + '"'
                datark["FG_PENGGANTI"] = '"' + str(move.l10n_id_tax_number[2:3] or 0) + '"'
                datark["NOMOR_FAKTUR"] = '"' + str(move.l10n_id_tax_number[3:] or 0) + '"'
                datark["TANGGAL_FAKTUR"] = '"' + str('{0}/{1}/{2}'.format(move.invoice_date.day, move.invoice_date.month, move.invoice_date.year)) + '"'
                datark["NOMOR_DOKUMEN_RETUR"] = '"' + str(move.efaktur_nomor_retur) + '"'
